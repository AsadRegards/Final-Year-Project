...
namespace FinalProject_PU
{
    ...
    public class Issuelocationpickup_Debris : Activity, IOnMapReadyCallback, ILocationSourceOnLocationChangedListener
    {
        ...
        private async void Set_location_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var Issue = JsonConvert.DeserializeObject<Model.Debris>(Intent.GetStringExtra("objtopass"));
                ...
                try
                {
                    ...
                    if (placemark != null)
                    {
                        ...
                        if (Issue.roadCoverage == "halfblocked")
                        {
                            ...
                            Issue.issueStatement = "Debris" + s_statement + "near" + LocationName;
                            await Control.IssueController.PostIssue<Model.Debris>(Issue, this);
                        }
                        else if (Issue.roadCoverage == "fullblocked")
                        {
                            s_statement = "is blocking full road";
                            Issue.issueStatement = "Debris" + s_statement +"near" + LocationName;
                            await Control.IssueController.PostIssue<Model.Debris>(Issue, this);
                        }
                        else if (Issue.roadCoverage == "sidearea")
                        {
