...
{
    [Activity(Label = "Issuelocationpickup_Garbage",NoHistory =true)]
    public class Issuelocationpickup_Garbage : Activity, IOnMapReadyCallback
    {
        ...
        private async void Set_location_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var p = JsonConvert.DeserializeObject<Model.Garbage>(Intent.GetStringExtra("objtopass"));
                p.locationLatitude = Final_Position.Latitude.ToString();
                p.locationLongitude = Final_Position.Longitude.ToString();
                p.Status = "unverified";
                p.issueFlag = new Control.IssueFlagDetector().DetectGarbageFlag(p);
                p.isresolved = 0;
                p.issueType = "Garbage";
                ...
                try
                {
                    ...
                    if (p.roadCoverage == "halfblocked")
                    {
                        ...
                        p.issueStatement = "Garbage " + s_statement + " near " + p.location_name;
                    ...
                    alert.Show();
                    await Control.IssueController.PostIssue<Model.Garbage>(p, this);
