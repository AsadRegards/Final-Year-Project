...
{
    [Activity(Label = "Issuelocationpickup_Garbage")]
    public class Issuelocationpickup_Garbage : Activity, IOnMapReadyCallback,ILocationSourceOnLocationChangedListener
    {
        ...
        private async void Set_location_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var Issue = JsonConvert.DeserializeObject<Model.Garbage>(Intent.GetStringExtra("objtopass"));
                ...
                try
                {
                    ...
                    if (placemark != null)
                    {
                        ...
                        string s_stat;
                        if (Issue.roadCoverage=="fullroad")
                        {
                            s_stat = "is blocking full road";
                            Issue.issueStatement = "Garbage" + "near" + LocationName;
                            await Control.IssueController.PostIssue<Model.Garbage>(Issue, this);

                        }
                        else if (Issue.roadCoverage == "halfroad")
                        {
                            s_stat = "is blocking half of road";
                            Issue.issueStatement = "Garbage" + "near" + LocationName;
                            await Control.IssueController.PostIssue<Model.Garbage>(Issue, this);

                        }
                        else if (Issue.roadCoverage == "smallarea")
                        {
                            s_stat = "is blocking small area of road";
                            Issue.issueStatement = "Garbage" + "near" + LocationName;
                            await Control.IssueController.PostIssue<Model.Garbage>(Issue, this);
