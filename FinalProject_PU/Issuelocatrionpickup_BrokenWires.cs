    ...
    public class Issuelocationpickup_BrokenWires : Activity, IOnMapReadyCallback
    {
        ...
        private async void Set_location_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                p.Status = "unverified";
                if (p.roadCoverage == "fullblock")
                    p.issueFlag = "red";
                if (p.roadCoverage == "halfblock")
                    p.issueFlag = "yellow";
                p.isresolved = 0;
                ...
                try
                {
                    ...
                    if (p.roadCoverage == "halfblock")
                    {
                        ...
                        p.issueStatement = "Broken Wires " + s_part + "near " + p.location_name;
                        
                    }
                    else if (p.roadCoverage == "fullblock")
                    {
                    ...
                    alert.Show();
                    await Control.IssueController.PostIssue<Model.Brokenpole>(p, this);
