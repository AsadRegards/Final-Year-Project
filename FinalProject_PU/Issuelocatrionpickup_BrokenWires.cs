...
{
    [Activity(Label = "Issuelocatrionpickup_BrokenWires")]
    public class Issuelocatrionpickup_BrokenWires : Activity, IOnMapReadyCallback, ILocationSourceOnLocationChangedListener
    {
        ...
        private async void Set_location_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var Issue = JsonConvert.DeserializeObject<Model.Brokenpole>(Intent.GetStringExtra("objtopass"));
                ...
                try
                {
                    ...
                    if (placemark != null)
                    {
                        ...
                        string s_part;
                        if (Issue.roadCoverage=="halfblock")
                        {
                            s_part = "Blocking half road";
                            Issue.issueStatement = "Broken Wires" + s_part + "near" + LocationName;
                            await Control.IssueController.PostIssue<Model.Brokenpole>(Issue, this);
                        }
                        else if (Issue.roadCoverage=="fullblock")
                        {
                            s_part = "Blocking full road";
                            Issue.issueStatement = "Broken Wires" + s_part+ "near" + LocationName;
                            await Control.IssueController.PostIssue<Model.Brokenpole>(Issue, this);
